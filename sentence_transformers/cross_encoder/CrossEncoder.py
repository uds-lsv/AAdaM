...
class CrossEncoder():
    ...
    def fit(self,
            ...
            use_amp: bool = False,
            log_callback: Callable[[float, int, int], None] = None,
            callback: Callable[[float, int, int, float], None] = None,
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for features, labels in tqdm(train_dataloader, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                ...
                if log_callback is not None:
                    log_callback(loss_value.item(), epoch, global_step)

                if evaluator is not None and evaluation_steps > 0 and training_steps % evaluation_steps == 0:
                    self._eval_during_training(evaluator, output_path, save_best_model, epoch, training_steps, global_step, callback)
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, global_step, callback):
        ...
        if evaluator is not None:
            ...
            if callback is not None:
                callback(score, epoch, global_step, self.best_score)
