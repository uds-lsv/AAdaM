...
def main() :
    ...
    if data_args.augmentation_file is not None:
        if 'stsb' in data_args.augmentation_file:
            aug_dataset = load_stsb_dataset( {"train": data_args.augmentation_file}, data_args.language)['train']
        elif 'translate' in data_args.augmentation_file:
            aug_dataset = load_str_dataset({"train": data_args.augmentation_file})['train']
        else:
            raise NotImplementedError

        raw_datasets['aug'] = aug_dataset
    ...
    aug_dataset = raw_datasets['aug'] if 'aug' in raw_datasets.keys() else None
    ...
    if aug_dataset is not None and training_args.do_train:
        warmup_trainer = trainer_class(
            model=model,
            args=training_args,
        ...
        _ = warmup_trainer.train()   # load_best_model_at_end
